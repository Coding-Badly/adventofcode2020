...
impl<'a> FieldIter<'a> {
    fn new(input: &'a str) -> Self {
        let indices: std::str::CharIndices = input.char_indices();
        Self {
            state: FieldIterState::Init,
            input,
            indices,
...
impl FieldIter<'_> {
    fn next_index(&mut self) -> Option<(usize, char)> {
        self.last_position = self.current_position;
        let rv = self.indices.next();
        if let Some((position, _)) = rv {
...
impl<'a> Iterator for FieldIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            match self.state {
                FieldIterState::Init => {
                    self.state = FieldIterState::ScanForKey;
                    self.key = None;
                    self.value = None;
                }
                FieldIterState::ScanForKey => {
                    if let Some((position, character)) = self.next_index() {
                        if character == '\n' {
                            break Some(FieldToken::EndOfRecord);
                        }
                        else if character.is_whitespace() {
                            continue;
                        }
                        else {
                            self.state = FieldIterState::GetKey(position);
                        }
                    }
                    else {
                        break None;
                ...
                FieldIterState::GetKey(key_start) => {
                    if let Some((_, character)) = self.next_index() {
                        if character == ':' {
                            if let Some(last_position) = self.last_position {
                                self.state = FieldIterState::StartValue;
                                self.key = Some(&self.input[key_start..last_position+1]);
                            }
                            else {
                                panic!("Internal error.  last_position not set.");
                            }
                        }
                        else if character.is_whitespace() {
                            panic!("Invalid key.  No terminating colon.");
                        }
                    }
                    else {
                        panic!("End of input encountered while parsing a key.");
                    }
                }
                FieldIterState::StartValue => {
                    if let Some((position, character)) = self.next_index() {
                        if character.is_whitespace() {
                            panic!("Missing value.");
                        }
                        else {
                            self.state = FieldIterState::GetValue(position);
                        }
                    }
                    else {
                        panic!("End of input encountered while parsing a value.");
                    }
                }
                FieldIterState::GetValue(value_start) => {
                    if let Some((_, character)) = self.next_index() {
                        if character.is_whitespace() {
                            if let Some(last_position) = self.last_position {
                                self.state = FieldIterState::EmitKeyValue;
                                self.value = Some(&self.input[value_start..last_position+1]);
                            }
                            else {
                                panic!("Internal error.  last_position not set.");
                            }
                ...
                FieldIterState::EmitKeyValue => {
                    self.state = FieldIterState::Init;
                    if let Some(key) = self.key.take() {
                        if let Some(value) = self.value.take() {
                            break Some(FieldToken::KeyValue(key, value));
                        }
                        else {
                            panic!("Internal error.  value not set.");
                        }
                    }
                    else {
                        panic!("Internal error.  key not set.");
                    }
                }
            }
        }
...
enum ValidationError {
    CantConvert(std::num::ParseIntError),
    OutOfRange,
    BadHeightFormat,
    BadHeightMetric,
    BadHeightImperial,
...
impl std::fmt::Display for ValidationError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::CantConvert(e) => write!(f, "conversion failed with error: {}", e),
            Self::OutOfRange => write!(f, "the value is out of range"),
            Self::BadHeightFormat => write!(f, "expected height (a number) followed by the units (\"cm\" or \"in\")"),
            Self::BadHeightMetric => write!(f, "metric heights must be at least 150 and at most 193"),
            Self::BadHeightImperial => write!(f, "imperial heights must be at least 59 and at most 76"),
            Self::BadHairColor => write!(f, "a hexadecimal color code was expected (\"#\" followed by a six digit hexadecimal number)"),
            Self::BadEyeColor => write!(f, "one of the following options was expected: \"amb\", \"blu\", \"brn\", \"gry\", \"grn\", \"hzl\", or \"oth\""),
            Self::BadPassportID => write!(f, "a nine digit number was expected"),
        }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut goal = std::collections::HashSet::<&str>::new();
    goal.insert("byr");
    goal.insert("iyr");
    goal.insert("eyr");
    goal.insert("hgt");
    ...
    let mut validators = std::collections::HashMap::<&str, Box<dyn Fn(&str) -> Result<(), ValidationError>>>::new();

    validators.insert("byr", Box::new(|value| {
    ...
    validators.insert("iyr", Box::new(|value| {
        let value = value.parse::<u16>()?;
    ...
    validators.insert("eyr", Box::new(|value| {
        let value = value.parse::<u16>()?;
    ...
    let re = Regex::new(r"^([1-9][0-9][0-9]?)(cm|in)$").unwrap();
    validators.insert("hgt", Box::new(move |value| {
        if let Some(captures) = re.captures(value) {
            if let Some(value) = captures.get(1) {
                let value = value.as_str().parse::<u16>()?;
                if let Some(units) = captures.get(2) {
                    if units.as_str() == "cm" {
                        if value >= 150 && value <= 193 {
                            ...
                            Err(ValidationError::BadHeightMetric)
                        }
                    }
                    else if units.as_str() == "in" {
                        if value >= 59 && value <= 76 {
                            Ok(())
                        }
                        else {
                            Err(ValidationError::BadHeightImperial)
                        }
                    }
                    else {
                        panic!("Internal error.  The unit for height must be \"cm\" or \"in\".");
                    }
                }
                else {
                    Err(ValidationError::BadHeightFormat)
                }
            }
            else {
                Err(ValidationError::BadHeightFormat)
    ...
    validators.insert("hcl", Box::new(move |value| {
        if re.is_match(value) {
            Ok(())
        }
        else {
            Err(ValidationError::BadHairColor)
        }
    }));

    let re = Regex::new(r"^amb|blu|brn|gry|grn|hzl|oth$").unwrap();
    validators.insert("ecl", Box::new(move |value| {
        if re.is_match(value) {
            Ok(())
        }
        else {
            Err(ValidationError::BadEyeColor)
        }
    }));

    let re = Regex::new(r"^[0-9]{9}$").unwrap();
    validators.insert("pid", Box::new(move |value| {
        if re.is_match(value) {
            Ok(())
        }
        else {
            Err(ValidationError::BadPassportID)
        }
    }));

    let mut have_all_count = 0;
    let mut valid_count = 0;
    let mut total_records = 0;
    let field_iter = FieldIter::new(INPUT);
    let mut found = std::collections::HashSet::<&str>::new();
    let mut valid = std::collections::HashSet::<&str>::new();
    for token in field_iter {
        match token {
            FieldToken::EndOfRecord => {
                if goal.difference(&found).count() == 0 {
                    have_all_count += 1;
                }
                if goal.difference(&valid).count() == 0 {
                    valid_count += 1;
                }
                total_records += 1;
                found.clear();
            ...
            FieldToken::KeyValue(key, value) => {
                found.insert(key);
                let is_field_valid = if let Some(validator) = validators.get(key) {
                    match validator(value) {
                        Ok(()) => { true }
                        Err(error) => { eprintln!("{} is not valid for {}: {}", value, key, error); false }
                    }
                }
                else {
                    true
    ...
    println!("have all fields = {}", have_all_count);
...
const INPUT: &str = "iyr:2015 cid:189 ecl:oth byr:1947 hcl:#6c4ab1 eyr:2026
hgt:174cm
pid:526744288
