...
const LETTER_Z: u32 = 'z' as u32;

fn anyone_yes() -> Result<(), Box<dyn std::error::Error>> {
    let mut total: u32 = 0;
    let mut group_answers: u32 = 0;
    for line in INPUT.lines().map(|line| line.trim()) {
        if line.len() > 0 {
            group_answers |= line.chars().fold(0u32, |accumulator, character| {
                let value = character as u32;
                if value >= LETTER_A && value <= LETTER_Z {
                    accumulator | (1 << (value - LETTER_A))
                }
                else {
                    panic!("Invalid input");
                }
            ...
            total += group_answers.count_ones();
            group_answers = 0;
        }
    }
    if group_answers > 0 {
    ...
    println!("anyone total = {}", total);

    Ok(())
}

fn everyone_yes() -> Result<(), Box<dyn std::error::Error>> {
    let mut total: u32 = 0;
    let mut mask: u32 = 0;
    let mut group: u32 = 0;
    for line in INPUT.lines().map(|line| line.trim()) {
        if line.len() > 0 {
            let answers = line.chars().fold(0u32, |accumulator, character| {
                let value = character as u32;
                if value >= LETTER_A && value <= LETTER_Z {
                    accumulator | (1 << (value - LETTER_A))
                }
...
const INPUT: &str = "gsvdkufnoawjmhp
wvhusojpnikgfadb
vshnpfedgwajkou
aujodhskfvnpgw
okpdnwhsfvjguqa
